...
/**
 * Decoder for serialized {@link org.versile.orb.entity.VBoolean} data.
 */
public final class VBooleanDecoder extends VEntityDecoderBase {

	VEntity entity = null;

	public VBooleanDecoder(VIOContext ctx, boolean explicit) {
		super(ctx, explicit);
	}
	...
	public boolean decodeHeader(VByteBuffer data) throws VEntityReaderException {
		...
		if (code == 0xf1)
			entity = new VBoolean(false);
		else if (code == 0xf2)
			entity = new VBoolean(true);
		else {
			failed = true;
			throw new VEntityReaderException("Invalid code for boolean type.");
		}
		return true;
	}
