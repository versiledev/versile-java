...
/**
 * Decoder for a serialized reference to a {@link org.versile.orb.entity.VObject} in a remote context.
 */
public final class VReferenceDecoder extends VEntityDecoderBase {
	VEntity entity = null;
	boolean have_code = false;
	int elements = 0;

	public VReferenceDecoder(VIOContext ctx, boolean explicit) {
		super(ctx, explicit);
	}
	...
	public boolean decodeHeader(VByteBuffer data) throws VEntityReaderException {
		...
		if (code == 0xfc)
			...
			throw new VEntityReaderException("Invalid code for remote reference to remote VObject");
		return true;
	}
	...
	public void putEmbeddedEntities(LinkedList<VEntity> entities)
		...
		try {
			entity = o_ctx.referenceFromPeerID(peer_id, true);
			o_ctx.referenceAddRecv(peer_id);
		} catch (VEntityError e) {
			throw new VEntityReaderException("Could not resolve VReference peer ID");
		}
	}
