...
/**
 * Consumer of byte data.
 *
 * <p>Can be connected to a {@link VByteProducer}. Implementing objects must
 * be connected to a {@link org.versile.reactor.VReactor} and be operated by
 * the reactor's main thread.</p>
 */
public interface VByteConsumer extends VConsumer {
	 ...
	 *
	 * <p>It is allowed to call {@link VByteProducer#canProduce(long)} on a connected
	 * producer within the scope this method call.</p>
	 *
	 * <p>The method should only be called by the owning reactor's main thread.</p>
	 *
	 * @param data buffer containing data to consume
	 * @return current cumulative consumption limit
	 * @throws IOException consumption error
	 */
	public long consume(VByteBuffer data)
			throws IOException;
	 ...
	 * data to {@link #consume(VByteBuffer)}. The consumer is responsible for
	 ...
	 */
	public void attach(VByteProducer producer)
			throws IOException;
